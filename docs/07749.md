...
> 原文:[https://hack aday . com/2017/11/20/分布式空气质量监测-通过出租车车队/](https://hackaday.com/2017/11/20/distributed-air-quality-monitoring-via-taxi-fleet/)
...
[![](../Images/d95c4c8abba7fb935447b77f153c7d45.png)](https://hackaday.com/wp-content/uploads/2017/11/pollution-heatmap.png) 这款出租车数据记录器的主要目的是收集空气中的颗粒物数量，并通过谷歌地图叠加显示污染水平。[詹姆斯]使用[一个光散射粒子传感器](http://www.mouser.com/ds/2/187/honeywell-hpm32322550b-1099596.pdf)和一个 Raspi 3 通过安卓系统将数据发送到云端。由于 Pi 只有一个本地 UART，[James]将其用于粒子传感器，并通过 FTDI 串行适配器连接数据密集型 GPS 模块。还有一个定位驾驶室的 GPS 和一个温度/湿度/压力传感器，以获得更全面的环境图片。
